...
static bool tud_audio_feature_unit_get_request(
    ...
    else if (UAC2_ENTITY_SPK_FEATURE_UNIT &&
...
void audio_task(void) {
    // When new data arrived, copy data from speaker buffer, to microphone
    // buffer and send it over Only support speaker & headphone both have the
    // same resolution If one is 16bit another is 24bit be care of LOUD noise !
    if (spk_data_size) {
